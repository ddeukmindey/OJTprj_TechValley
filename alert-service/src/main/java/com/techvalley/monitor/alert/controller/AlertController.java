 ...
 * Toàn bộ business logic nằm ở AlertService (đúng Layered Architecture trong
 * AGENTS.md).
 */
@RestController
...
public class AlertController {
  ...
  @GetMapping
  @Operation(summary = "Xem lịch sử cảnh báo", description = "Xem tất cả hoặc lọc theo loại, ngày, tháng, khoảng ngày, trạng thái xử lý, instance.")
  public ResponseEntity<ApiResponse<PageResponse<AlertResponse>>> getAlerts(
      @Parameter(description = "Bộ lọc: alertType, instanceId, isResolved, date, month, fromDate, toDate, page, size, sortBy, sortOrder") @ModelAttribute AlertFilterRequest filter) {
  ...
  @Operation(summary = "Đánh dấu cảnh báo đã xử lý", description = "Chuyển 1 cảnh báo đang ở trạng thái chưa xử lý (isResolved = false) sang đã xử lý.")
  public ResponseEntity<ApiResponse<AlertResponse>> resolveAlert(@PathVariable @NonNull Long id) {
