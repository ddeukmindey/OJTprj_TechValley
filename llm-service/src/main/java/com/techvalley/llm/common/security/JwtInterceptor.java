...
/**
 * Xác thực JWT y hệt các service khác (cùng jwt.secret trong application.yml
 * của mọi service, vì auth-gateway-service phát hành 1 token dùng chung toàn hệ thống).
 * Khác biệt duy nhất so với alert-service/monitoring-service: lưu thêm rawToken vào
 * UserContextInfo để DiagnosisServiceImpl forward tiếp cho instance-service/alert-service.
 */
@Component
public class JwtInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        try {
            ...
            UserContext.set(new UserContextInfo(memberId, email, role, token));
            return true;
