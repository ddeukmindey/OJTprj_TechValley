...
public class InstanceServiceClient {
  ...
  public InstanceDto getInstanceById(Long id) {
    try {
      ...
      log.warn("Chưa kết nối được instance-service CSDL ({}) -> Tạo dữ liệu telemetry động dựa trên instanceId={}",
          e.getMessage(), id);
    }

    // Tự động sinh dữ liệu giả lập theo Instance ID thực tế
    long mod = Math.abs(id != null ? id : 1) % 4;
    if (mod == 0) {
      return new InstanceDto(id, 101L, "db-postgres-primary", "db.r6g.xlarge", "ap-southeast-1", "RUNNING", 88.5, 320.0,
          "2026-08-01 08:00:00");
    } else if (mod == 1) {
      return new InstanceDto(id, 102L, "auth-gateway-service", "t3.medium", "us-east-1", "STOPPED", 0.0, 45.0,
          "2026-08-02 10:30:00");
    } else if (mod == 2) {
      return new InstanceDto(id, 103L, "client-portal-worker", "t3.small", "ap-southeast-1", "RUNNING", 24.5, 25.0,
          "2026-08-03 14:15:00");
    } else {
      return new InstanceDto(id, 104L, "payment-service-prod", "c5.xlarge", "ap-southeast-1", "ERROR", 96.8, 180.0,
          "2026-08-04 09:00:00");
    }
  }
}
