...
- **Thư viện Tự động Sinh Mã Nguồn (Code Generation):** 100% các module Backend BẮT BUỘC sử dụng **Lombok** (`@Data`, `@Builder`, `@NoArgsConstructor`, `@AllArgsConstructor`, `@Slf4j`, `@RequiredArgsConstructor`) để tự động sinh Getter, Setter, Constructors, Builder pattern và Logger Instance, tuyệt đối không viết thủ công code boilerplate.
...
- **Mô tả:** Lấy danh sách các instance bị tạm dừng (`status == 'STOPPED'`) ít nhất 48 giờ mà chưa bật lại.
...
| Thành viên Đảm nhiệm | Mô-đun Chức năng | Chi tiết Công việc & Các API Phụ trách |
| :--- | :--- | :--- |
| **Đinh Thị Diệu Châu** | **2.1. Nhóm chức năng Auth (Auth & Security Module)** | • **Auth:** Triển khai API `POST /api/auth/login` (Đăng nhập hệ thống).<br>• **Security Middleware:** Xác thực JWT Token & Phân quyền RBAC cho mọi request trong hệ thống.<br>• **Tài liệu:** Đóng góp báo cáo/slide phần Authentication & Security. |
| **Phan Thị Cẩm Ly** | **2.2. Nhóm chức năng Instance (Instance Management Module)** | • **CRUD Instance:** `POST /api/instances` (Đăng ký instance mới), `GET /api/instances` (Danh sách có phân trang, filter, sort), `GET /api/instances/{id}` (Lấy chi tiết), `PATCH /api/instances/{id}/status` (Cập nhật trạng thái), `DELETE /api/instances/{id}` (Xóa instance).<br>• **Business Rule:** Logic chặn xóa instance khi ở trạng thái `RUNNING` (HTTP 400).<br>• **Tài liệu:** Đóng góp báo cáo/slide phần Instance Management. |
| **Thân Đức Minh** *(BẠN)* | **2.3. Nhóm chức năng Monitoring (Monitoring System Module)** | • **Giám sát Hạ tầng:** `GET /api/monitor/warnings` (Lấy cảnh báo CPU cao, tự động ghi alert), `GET /api/monitor/errors` (Lấy cảnh báo status ERROR, tự động ghi alert `ERROR_DETECTED`), `GET /api/monitor/long-stopped` (Lấy instance bị STOPPED ít nhất 48h), `GET /api/monitor/report` (Báo cáo tổng hợp trạng thái hạ tầng).<br>• **Business Rule:** Logic quét tự động & chống trùng lặp Alert (Alert Deduplication Check).<br>• **Tài liệu:** Đóng góp báo cáo/slide phần Monitoring System. |
| **Lê Thị Thuý Hằng** | **2.4. Nhóm chức năng Cảnh báo (Alert System Module)** | • **Quản lý Cảnh báo:** `GET /api/alerts` (Xem lịch sử cảnh báo có filter theo loại, theo ngày, khoảng thời gian A->B, đã xử lý/chưa xử lý), `PATCH /api/alerts/{id}/resolve` (Đánh dấu cảnh báo đã xử lý).<br>• **Tài liệu:** Đóng góp báo cáo/slide phần Alert System Module. |
| **Trịnh Lương Việt** | **2.5. Nhóm chức năng Quản lý Khách hàng & Chi phí, SLA (Client, Cost & SLA Module)** | • **Quản lý Client:** `POST /api/clients` (Đăng ký khách hàng), `GET /api/clients` (Danh sách tất cả khách hàng), `GET /api/clients/{id}/instances` (Danh sách instance của 1 client).<br>• **Phân tích Tài chính & SLA:** `GET /api/clients/{id}/cost` (Tính chi phí), `GET /api/clients/{id}/cost-forecast` (Dự báo chi phí tháng tới), `GET /api/clients/{id}/sla` (Tính SLA uptime).<br>• **Tài liệu & System Lead:** Quản lý Swagger/OpenAPI UI, Git Flow & PPT Demo. |
| *(Chưa triển khai)* | **2.6. Nhóm chức năng LLM (AI Feature)** | • **Mô hình AI:** `GET /api/instances/{id}/diagnosis` (Tự động chẩn đoán sự cố bằng AI).<br>• **Trạng thái:** Tạm thời chưa triển khai (Dành cho giai đoạn mở rộng tiếp theo). |
